...
function ContactForm() {
  ...
  const { handleSubmit: submitForm, loading, error } = useContactForm();
  ...
  return (
    <form
      ...
      className="w-full max-w-xl mx-auto px-4 sm:px-0 space-y-5 sm:space-y-6"
    >
      <div>
        <input
          {...register("name")}
...
dark:bg-gray-900 dark:border-gray-700 dark:text-white"
      ...
      <button
        type="submit"
        disabled={loading}
        className={`w-full sm:w-auto px-6 py-2.5 rounded-lg text-white transition-all
  ${
    loading
